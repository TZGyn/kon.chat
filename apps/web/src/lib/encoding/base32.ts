...
export function encodeBase32UpperCaseNoPadding(
	bytes: Uint8Array,
): string {
	return encodeBase32_internal(
		bytes,
...
export function encodeBase32LowerCaseNoPadding(
	bytes: Uint8Array,
): string {
	return encodeBase32_internal(
		bytes,
	...
	for (let i = 0; i < bytes.byteLength; i += 5) {
		...
		for (let j = 0; j < 8; j++) {
			if (bufferBitSize >= 5) {
				result +=
					alphabet[
						Number((buffer >> BigInt(bufferBitSize - 5)) & 0x1fn)
					]
				bufferBitSize -= 5
			} else if (bufferBitSize > 0) {
				result +=
					alphabet[
						Number((buffer << BigInt(6 - bufferBitSize)) & 0x3fn)
					]
				bufferBitSize = 0
...
export function decodeBase32IgnorePadding(
	encoded: string,
): Uint8Array {
	return decodeBase32_internal(
		encoded,
...
function decodeBase32_internal(
	...
	for (let i = 0; i < encoded.length; i += 8) {
		...
		for (let j = 0; j < 8; j++) {
			...
			chunk |=
				BigInt(decodeMap[encoded[i + j]]) << BigInt((7 - j) * 5)
			bitsRead += 5
		}
		if (bitsRead < 40) {
			let unused: bigint
		...
		for (let i = 0; i < byteLength; i++) {
			result[totalBytes] = Number(
				(chunk >> BigInt(32 - i * 8)) & 0xffn,
			)
			totalBytes++
		}
	}
